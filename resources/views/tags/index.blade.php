<x-app-layout>
    <x-slot name="header">
        ...
        <button class="btn--primary ms-auto" x-data=""
                               x-on:click="$dispatch('open-modal', 'form-new-tag-group'); $dispatch('button-new-tag-group-click', ''); $dispatch('reset-form-tag-group');">
    ...
    <div class="py-12" x-data="tagIndex()" x-init="flashMessage()"
         @button-new-tag-group-click.window="formNewTagGroupProps.action = '{{ route('tag-groups.store') }}'; formNewTagGroupProps.method = 'post'; formNewTagGroupProps.buttonText = '{{ __('Save') }}';"
         @reset-form-tag-group.window="resetSelectedTagGroup()">
                ...
                <div class="bg-white shadow sm:rounded-lg relative overflow-hidden p-6 gap-2 flex flex-col">
                    ...
                    <div class="absolute right-0 top-0 mt-2 me-2 gap-1.5 flex">
                        <button class="shadow-sm hover:shadow bg-yellow-500/60 backdrop-blur-xl hover:bg-yellow-600/80 p-2 rounded-lg text-white"
                            x-on:click="formNewTagGroupProps.action = '{{ route('tag-groups.update', $tagGroup) }}'; formNewTagGroupProps.method = 'patch'; formNewTagGroupProps.buttonText = '{{ __('Update') }}'; selectedTagGroup = JSON.parse('{{ $tagGroup }}'); $dispatch('open-modal', 'form-new-tag-group');">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        ...
                        <button class="shadow-sm hover:shadow bg-red-500/60 backdrop-blur-xl hover:bg-red-600/80 p-2 rounded-lg text-white"
                                x-on:click="formNewTagGroupProps.action = '{{ route('tag-groups.update', $tagGroup) }}'; formNewTagGroupProps.method = 'patch'; formNewTagGroupProps.buttonText = '{{ __('Update') }}'; selectedTagGroup = JSON.parse('{{ $tagGroup }}'); $dispatch('open-modal', 'confirm-tag-group-deletion');">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <x-modal name="form-new-tag-group">
            <form method="post" class="p-6" :action="formNewTagGroupProps.action" focusable>
                @csrf
                <input hidden name="_method" x-model="formNewTagGroupProps.method">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                    <template x-if="selectedTagGroup.id === null">
                        <span>{{ __('New Tag Group') }}</span>
                    </template>
                    <template x-if="selectedTagGroup.id !== null">
                        <span>{{ __('Update Tag Group ') }}<span x-text="selectedTagGroup.name"></span></span>
                    </template>
                </h2>
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Tag Group *</label>
                    <input type="text" x-model="selectedTagGroup.name" id="name" name="name" class="w-full border rounded p-2">
                </div>
                <button type="submit"
                        class="btn--primary"
                        :disabled="selectedTagGroup.name === null || selectedTagGroup.name.length === 0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                    <span x-text="formNewTagGroupProps.buttonText"></span>
                </button>
            </form>
        </x-modal>
        ...
        <x-modal name="confirm-tag-group-deletion" focusable>
            <form method="post" x-ref="form" class="p-6" :action="formNewTagGroupProps.action">
                @csrf
                @method('delete')

                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                    {{ __('Are you sure you want to delete tag group named ') }}<span x-text="selectedTagGroup.name"></span>{{ __('?') }}
                </h2>
